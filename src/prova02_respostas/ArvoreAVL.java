...
//		System.out.println(" -> " + resultado);
	...
	public ArvoreAVL clone() {
	    // Cria uma nova árvore AVL e clona os nós
	    ArvoreAVL novaArvore = new ArvoreAVL();
	    if (this.raiz != null) {
	        novaArvore.raiz = clonarNodo(this.raiz, null);
	    }
	    return novaArvore;
	}

	private NodoAVL clonarNodo(NodoAVL nodoOriginal, NodoAVL paiNovo) {
	    if (nodoOriginal == null) {
	        return null;
	    }
	    // Cria um novo nó com os mesmos valores do nó original
	    NodoAVL novoNodo = new NodoAVL(nodoOriginal.getChave(), nodoOriginal.getValor());
	    novoNodo.setPai(paiNovo);
	    novoNodo.setEsquerdo(clonarNodo(nodoOriginal.getEsquerdo(), novoNodo));
	    novoNodo.setDireito(clonarNodo(nodoOriginal.getDireito(), novoNodo));
